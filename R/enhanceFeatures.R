...
#' @param nthread Number of threads to use for xgboost. By default, 1 thread is
#'   used.
#'
...
.enhance_features <- function(
  ...
  nrounds, train.n, nthread = 1
    ...
    if (!all(colnames(X.enhanced) == colnames(X.ref))) {
        warning(
            "colnames of reducedDim and X.enhanced do not match.\n",
    ...
    for (feature in feature_names) {
        ...
        if (nrounds == 0) {
            data.train <- xgb.DMatrix(
                data = X.ref[train.index, ],
                label = Y.ref[feature, train.index]
            )
            data.test <- xgb.DMatrix(
                data = X.ref[-train.index, ],
                label = Y.ref[feature, -train.index]
            )
            ...
            fit.train <- xgb.train(
                ...
                evals = evals, verbose = 0
            )
            nrounds <- which.min(attr(fit.train, "evaluation_log")$test_rmse)
        }

        data.full <- xgb.DMatrix(data = X.ref, label = Y.ref[feature, ])
        evals <- list(train = data.full)
        ...
        rmse[feature] <- attr(fit, "evaluation_log")$train_rmse[nrounds]
    }
