...
// [[Rcpp::export]]
List iterate_deconv_refactor(arma::mat Y, List df_j, bool tdist, int nrep, int n, int n0, 
  ...
  for (int i = 1; i < nrep; i++) {
    ...
    for (int j0 = 0; j0 < n0; j0++){
      ...
      vec p_new = {0.0};
      for (int r = 0; r< subspots; r++){
        p_prev += dmvnrm_arma_fast(Y_j_prev.row(r), vectorise(mu_i_j.row(r)), sigma_i/w[j0 + n0*r], true) - c*(accu(pow(Y_j_prev.row(r)-Y0.row(j0),2)));
        p_new  += dmvnrm_arma_fast(Y_j_new.row(r),  vectorise(mu_i_j.row(r)), sigma_i/w[j0 + n0*r], true) - c*(accu(pow(Y_j_new.row(r) -Y0.row(j0),2)));
      }
    ...
    for (int j = 0; j < n; j++){
      ...
      double h_z_new;
      if (j_vector.size() != 0){
        h_z_prev = gamma/j_vector.size() * 2*accu((z(j_vector) == z_j_prev)) + dmvnrm_arma_fast(Y.row(j), vectorise(mu_i.row(z_j_prev-1)), sigma_i/w[j], true)[0];
        h_z_new  = gamma/j_vector.size() * 2*accu((z(j_vector) == z_j_new )) + dmvnrm_arma_fast(Y.row(j), vectorise(mu_i.row(z_j_new -1)), sigma_i/w[j], true)[0];
      } else {
        h_z_prev = dmvnrm_arma_fast(Y.row(j), vectorise(mu_i.row(z_j_prev-1)), sigma_i/w[j], true)[0];
        h_z_new  = dmvnrm_arma_fast(Y.row(j), vectorise(mu_i.row(z_j_new -1)), sigma_i/w[j], true)[0];
      }
