update_mu_lambda = function(prev, index, l0, lm0, data, b, a_n){
  lambda_prev = prev[3]
  index_1 = prev[4:length(prev)] == 1
  n_i = sum(index_1)
  mean_i = (lm0 + lambda_prev*sum(data[index_1,"Y"]))/(l0 + n_i*lambda_prev)
  var_i = 1/(l0 + n_i * lambda_prev)
  mu_i = rnorm(1, mean = mean_i, sd = sqrt(var_i))
  beta_i = b + sum((data[,"Y"]-mu_i*index_1)^2)/2
  lambda_i = rgamma(1, a_n, beta_i)
  return(list(mu = mu_i, lambda = lambda_i))
}

update_z = function(data_sim, index, jndex, data, g, mu_i, lambda_i, dj){
  z_j_prev = data_sim[index,jndex+3]
  z_j_new = z_j_prev*-1
  x_j = data[jndex,"x"]
  y_j = data[jndex,"y"]
  j_vector = df_j[[jndex]]
  h_z_prev = g/length(j_vector)* sum(z_j_prev * data_sim[index, j_vector]) + dnorm(data[jndex,"Y"], mean = mu_i * (z_j_prev == 1), 
                                                                                             sd = 1/sqrt(lambda_i), log = T)
  h_z_new = g/length(j_vector) * sum(z_j_new * data_sim[index, j_vector]) + dnorm(data[jndex,"Y"], mean = mu_i * (z_j_new == 1), 
                                                                                            sd = 1/sqrt(lambda_i), log = T)
  prob_j = min(exp(h_z_new - h_z_prev),1)
  sample(x = c(z_j_new, z_j_prev), size = 1, prob = c(prob_j, 1-prob_j))
}
...
run_mcmc = function(df, nrep = 1000, mu0 = mean(df[,"Y"]), lambda0 = 1/100, alpha = 1, beta = 0.01, gamma = 2, seed = 100){
  ...
  #Iterate
  for (i in 2:nrep){
    #update mu then lambda
    mulambda = update_mu_lambda(prev = df_sim[i-1,], index = i, l0 = lambda0, 
                                lm0 = lambdamu0, data = df, 
                                b = beta, a_n = alpha_n)
    df_sim[i, 2] = mulambda$mu
    #update lambda
    df_sim[i,3] = mulambda$lambda
    #update z
    df_sim[i, 4:ncol(df_sim)] = df_sim[i-1, 4:ncol(df_sim)]
    for (j in 1:n){
      df_sim[i, j+3] = update_z(data_sim = df_sim, index = i, jndex = j, data = df, g = gamma,
                                mu_i = mulambda$mu, lambda_i = mulambda$lambda, dj = df_j)
    }
  }
  return(df_sim)
}
