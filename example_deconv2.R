...
df2 = df[rep(seq_len(n), 7), ] #rbind 7 times
df2$j2 = rep(1:(7*n))
...
rect_center = sapply(1:10,function(x)df2[sample(7*n, 1) ,c("x","y")])
for(i in 1:10){
...
circ_center = sample(4:16, 2, replace = T)
r = 3 #radius
s = seq(0,r,1/4) #sagitta
...
p1 = ggplot(data = df2, aes(x = x, y = y))+
  geom_text(aes(color = factor(z), label = "\u2B23"), 
            size = 7, family = "Lucida Sans Unicode")+
  labs(color = "State")+theme_classic()+
  guides(shape = F, size = F, alpha = F) +
  geom_text(data = df, aes(x = x+0.05, y = y+0.2), size = 20, label = "\u2B21", alpha = 0.3, family = "Lucida Sans Unicode")
...
p4 = ggplot(data = as.data.frame(df2sum), aes(x = x, y = y))+
  geom_text(aes(color = Y1, label = "\u2B22"), 
            size = 20, family = "Lucida Sans Unicode")+
  labs(color = expression(Y[1]))+theme_classic()+
  scale_color_distiller(palette = "Blues", direction = 1, breaks = c(0,35)) +
  guides(shape = F, size = F)

p5 = ggplot(data = as.data.frame(df2sum), aes(x = x, y = y))+
  geom_text(aes(color = Y2, label = "\u2B22"), 
            size = 20, family = "Lucida Sans Unicode")+
  labs(color = expression(Y[2]))+theme_classic()+
  scale_color_distiller(palette = "Blues", direction = 1, breaks = c(0,35)) +
