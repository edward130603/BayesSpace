...
data1 = read_delim("data-raw/PDAC/GSM3405534_PDAC-B-ST1-filtered.txt", "\t", escape_double = FALSE, trim_ws = TRUE)
#data2 = read.delim("data-raw/PDAC/GSM3036911.tsv", stringsAsFactors=FALSE)
rowdata1 = data1$Genes
...
#Make sce
sce_B = SingleCellExperiment(assays = list(counts = data1), colData = positions)

#Preprocess
df_qc = perCellQCMetrics(sce_B)
qc = quickPerCellQC(df_qc)
ggplot(positions, aes(x, -y, col = qc$discard))+
  geom_point(size = 5)+coord_fixed()
sce_B$discard = qc$discard

#Normalization
set.seed(100)
clusters = quickCluster(sce_B)
sce_B = computeSumFactors(sce_B, clusters = clusters)
sce_B = logNormCounts(sce_B)
plot(librarySizeFactors(sce_B), sizeFactors(sce_B), pch=16,
     xlab="Library size factors", ylab="Deconvolution factors", log="xy", 
     col = clusters)

set.seed(101)
dec <- modelGeneVarByPoisson(sce_B)
top <- getTopHVGs(dec, prop=0.1)
plot(dec$mean, dec$total, pch=16, cex=0.5,
     xlab="Mean of log-expression", ylab="Variance of log-expression",
     col = ifelse(sce_B$discard, "red", "black"))
curfit <- metadata(dec)
curve(curfit$trend(x), col='dodgerblue', add=TRUE, lwd=2)

#Dim reduction and clustering
set.seed(102)
sce_B <- denoisePCA(sce_B, technical=dec, subset.row=top)
PCs = getDenoisedPCs(sce_B, technical=dec, subset.row=top, min.rank = 5, max.rank = 50)
sce_B <- runTSNE(sce_B, dimred="PCA")
sce_B <- runUMAP(sce_B, dimred="PCA")

snn.gr <- buildSNNGraph(sce_B, use.dimred="PCA", k=10)
sce_B$cluster <- factor(igraph::cluster_walktrap(snn.gr)$membership)
table(sce_B$cluster)

ggplot(as.data.frame(colData(sce_B)), aes(x = x, y = -y, fill = logcounts(sce_B)[grep("S100A4", rownames(sce_B)),])) + 
  geom_point(pch = 22, size = 7)+
  labs(x = NULL, y = NULL, fill = "Cluster") +
  scale_fill_viridis(option = "A")+
  theme_void()+ coord_fixed()
ggplot(as.data.frame(colData(sce_B)), aes(x = x, y = y, fill = reducedDim(sce_B, "TSNE")[,2])) + 
  geom_point(pch = 22, size = 7)+
  labs(x = NULL, y = NULL, fill = "Cluster") +
  scale_fill_viridis(option = "A")+
  theme_void()+ coord_fixed()
ggplot(as.data.frame(colData(sce_B)), aes(x = x, y = y, fill = librarySizeFactors(sce_B))) + 
  geom_point(pch = 22, size = 7)+
...
#Run mcmc
km6 = kmeans(reducedDim(sce_B, "PCA"), centers = 6)$cluster
km4 = kmeans(reducedDim(sce_B, "PCA"), centers = 4)$cluster
km3 = kmeans(reducedDim(sce_B, "PCA"), centers = 3)$cluster
km7 = kmeans(reducedDim(sce_B, "PCA"), centers = 7)$cluster
km8 = kmeans(reducedDim(sce_B, "PCA"), centers = 8)$cluster
km5 = kmeans(reducedDim(sce_B, "PCA"), centers = 5)$cluster
km9 = kmeans(reducedDim(sce_B, "PCA"), centers = 9)$cluster
km10 = kmeans(reducedDim(sce_B, "PCA"), centers = 10)$cluster
clust3 = cluster(Y= PCs$components[,1:15], positions = as.matrix(positions), q = 3, init = km3, nrep = 10000, gamma = 1, dist = 1)
clust4 = cluster(Y= PCs$components[,1:15], positions = as.matrix(positions), q = 4, init = km4, nrep = 10000, gamma = 1, dist = 1)
clust4v2 = cluster(Y= cbind(librarySizeFactors(sce_B), PCs$components[,1:15]), positions = as.matrix(positions), q = 6, init = km6, nrep = 10000, gamma = 1, dist = 1)
...
clust4v2_alpha = pmax(colMeans(clust4v2$z[9000:10000,]==1),
                    colMeans(clust4v2$z[9000:10000,]==2),
                    colMeans(clust4v2$z[9000:10000,]==3),
                    colMeans(clust4v2$z[9000:10000,]==4))
...
ggplot(as.data.frame(colData(sce_B)), aes(x = x, y = y, fill = factor(km3))) + 
  geom_point(size = 7, pch = 22)+
  labs(x = NULL, y = NULL, color = "Cluster") +
  #scale_fill_viridis_d(option = "A")+
  theme_void()+ coord_fixed()
ggplot(as.data.frame(colData(sce_B)), aes(x = x, y = y, fill = factor(clust6col))) + 
  geom_point(size = 7, pch = 22)+
  labs(x = NULL, y = NULL, color = "Cluster") +
  #scale_fill_viridis_d(option = "A")+
  theme_void()+ coord_fixed()
ggplot(as.data.frame(colData(sce_B)), aes(x = x, y = y, fill = factor(clust4col), alpha = clust4_alpha)) + 
  geom_point(size = 7, pch = 22)+
  labs(x = NULL, y = NULL, fill = "Cluster") +
  guides(alpha = F)+
...
#Deconvolution
df = data.frame(PCs$components[,1:15])
colnames(df) = paste0("Y",1:15)
df$x = positions$x
df$y = positions$y
df$j = 1:nrow(df)
saveRDS(clust4, "data-raw/clust4_pancreasB.RDS")
saveRDS(df, "data-raw/df_pancreasB.RDS")
less deconv1 = deconvolve(Y= PCs$components[,1:15], positions = positions, q = 4, init = rep(clust4col, 9), nrep = 25000, gamma = 1, dist = 0.34)
...
deconv1_col = apply(deconv1$z[15000:25000,], 2, Mode)
deconv2_col = apply(deconv1$z[1000:5000,], 2, Mode)
...
ggplot(positions2, aes(x = x, y = y, fill = factor(deconv1_col), alpha =1)) + 
