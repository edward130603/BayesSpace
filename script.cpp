...
// [[Rcpp::export]]
void iterate_deconv(mat Y, List df_j, int nrep, int n, int n0, int d, double gamma, int q, vec init, NumericVector mu0, mat lambda0, double alpha, double beta){
  ...
  mat error_var = diagmat(one_vec)/d/d; 
  for (int i = 1; i < nrep; i++){
...
/*** R
# iterate_deconv(Y = Y, df_j = df_j, nrep = nrep, n = n, n0 = n0, d = d, gamma = gamma, q = q, init = init, mu0 = mu0, lambda0 = lambda0, alpha = alpha, beta = beta)
*/
