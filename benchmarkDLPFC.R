...
colnames(df_151673) = c(paste0("Y", 1:15, "_PCA"),  
                        paste0("Y", 1:2 , "_TSNE2"),
                        paste0("Y", 1:3 , "_TSNE3"),
                        paste0("Y", 1:2 , "_UMAP2"),
                        paste0("Y", 1:3 , "_UMAP3"))
...
for (i in 1:5){
  if (input == "PCA"){
    clust = run_mcmc_multi(df = df[,paste0("Y", 1:n, "_PCA")], 
                           gamma = 4, 
                           q = q[i],
                           d = n,
                           nrep = 5000,
                           init = df[,paste0("kmeans", d[i])])
    saveRDS(clust, paste0("clust_PCA_d", n, "_q", q[i]))
  } else {
    clust = run_mcmc_multi(df = df[,paste0("Y", 1:n, "_", input, "n")], 
                           gamma = 4, 
                           q = q[i],
                           d = n,
                           nrep = 5000,
                           init = df[,paste0("kmeans", d[i])])
    saveRDS(clust, paste0("clust_", input, n, "_q", q[i]))
  }
}
