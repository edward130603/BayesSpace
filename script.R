...
run_mcmc_multi = function(df, nrep = 1000, q = 3, d = 2, mu0 = colMeans(df[,grep("Y",colnames(df))]), lambda0 = diag(0.01, nrow = d), alpha = 1, beta = 0.01, gamma = 2, init = rep(1,nrow(df)), seed = 100){
  ...
  for (i in 2:nrep){
    ...
    for (j in 1:n){
      ...
      j_vector = df_j[[j]]
      h_z_prev = gamma/length(j_vector)* 2*sum(((z_j_prev == df_sim_z[i, j_vector])-0.5)) + mvtnorm::dmvnorm(Y[j,], mean = mu_i[z_j_prev,], sigma = sigma_i, log = T)
      h_z_new = gamma/length(j_vector) * 2*sum(((z_j_new  == df_sim_z[i, j_vector])-0.5)) + mvtnorm::dmvnorm(Y[j,], mean = mu_i[z_j_new, ], sigma = sigma_i, log = T)
      if ((i %% 10) == 0){
        logLikj[j] = sum(sapply(1:q, function(x){gamma/length(j_vector)* 2*sum(((x == df_sim_z[i, j_vector])-0.5)) + mvtnorm::dmvnorm(Y[j,], mean = mu_i[x,], sigma = sigma_i, log = T)}))
      }
    ...
    if (i %% 10 == 0){logLik[i/10] = plogLik[i]-sum(logLikj)}
    if (i %% 100 == 0){print(paste0("Iteration: ",i))}
  }
   return(list(z = df_sim_z, mu = df_sim_mu, lambda = df_sim_lambda, plogLik = plogLik, logLik = logLik))
}
...
run_mcmc_deconv = function(df, nrep = 1000, q = 3, d = 2, mu0 = colMeans(df[,grep("Y",colnames(df))]), lambda0 = diag(0.01, nrow = d), alpha = 1, beta = 0.01, gamma = 2, seed = 100, prev){
  ...
  for (i in 2:nrep){
    ...
    for (j in 1:n0){
      ...
      error = scale(mvtnorm::rmvnorm(n = 4, rep(0, d), sigma = diag(d)/5), scale = F)
      Y_j_new = Y_j_prev + error
      mu_i_four = mu_i[df_sim_z[i-1,j + 0:3 * n0],]
      p_prev = prod(sapply(1:4, function(x){mvtnorm::dmvnorm(Y_j_prev[x,], mu_i_four[x,], sigma_i)}))
      p_new = prod(sapply(1:4, function(x){mvtnorm::dmvnorm(Y_j_new[x,], mu_i_four[x,], sigma_i)}))
