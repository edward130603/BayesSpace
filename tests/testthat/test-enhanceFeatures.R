...
test_that("genes are predicted with xgboost", {
  ...
  Y.enhanced <- .xgboost_enhance(X.ref, X.enhanced, Y.ref, c(gene), tune.nrounds = FALSE)
...
test_that("genes are predicted with tuned xgboost", {
  ...
  data.train <- xgb.DMatrix(data = X.ref[train.index, ],  label = Y.ref[gene, train.index])
  data.test  <- xgb.DMatrix(data = X.ref[-train.index, ], label = Y.ref[gene, -train.index])
  ...
  fit <- xgboost(data=X.ref, label=Y.ref[gene, ], objective="reg:squarederror",
                 max_depth=2, eta=0.03, nrounds=nrounds, nthread=1, verbose=FALSE)
  new <- predict(fit, newdata=X.enhanced)
  set.seed(100)
  Y.enhanced <- .xgboost_enhance(X.ref, X.enhanced, Y.ref, c(gene), tune.nrounds = TRUE, train.n = 64)
