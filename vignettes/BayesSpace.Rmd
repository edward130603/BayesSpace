...
```{r cluster}
# sce <- spatialCluster(sce, q=4, init.method="mclust", model="t", nrep=1000, save.chain=TRUE)
sce$spatial.cluster <- floor(runif(ncol(sce), 1, 5))
```
...
enhanced <- spatialEnhance(sce, q=4, init=sce$spatial.cluster, model="t", nrep=1000, save.chain=TRUE)
```

We can plot the enhanced cluster assignments as above.
```{r enhance.plot, eval=FALSE}
